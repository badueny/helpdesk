...
class TicketsController extends Controller
{
    ...
    public function index(){
        if(Auth::user()->hasRole('admin')){
            ...
            $open = Tickets::where(['assigned_to' => Auth::id()])->orWhere(['user_id'=> Auth::id()])->get(['status' => 'open'])->count();
    ...
    public function store(Request $request){
        ...
        if($settings->ticket_email == 'yes'){
            ...
            Mail::send('mails.thanks',['ticket'=>$request, 'department' => $department], function ($message) use ($settings){
                $message->from('helpdesk@bantenprov.go.id', 'Helpdesk Pemprov Banten ');
                $message->subject('Tiket Baru Telah dibuat');
